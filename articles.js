(function () {
  ...
  function createXML(articles, link, atomLink) {
    var str = '<?xml version="1.0" encoding="UTF-8"?>\n'
      ...
      +' <link>' + link + '</link>\n'
      +' <atom:link type="application/rss+xml" href="' + atomLink + '" rel="self"></atom:link>\n'
    ...
    for(var i = 0; i < articles.length; i = i + 1) {
      ...
      + (articles[i].text || '')
      + ']]></content:encoded>\n'
      +' <pubDate>'+ articles[i].date +'</pubDate>\n'
      +' <guid>' + articles[i].id + '</guid>\n'
  ...
  RemoteStorage.defineModule(moduleName, function(privateClient, publicClient) {
    ...
    return {
      exports: {
        ...
        updateFeed: function(destination) {
          return this.getAll().then(function (allArticles) {
            return Object.keys(allArticles).map(function(key) {
              return allArticles[key];
            ...
            return createXML(articles, 'http://example.com', "no se");
          })
          .then(function(xml){
            return publicClient.storeFile('application/rss+xml', destination, xml);
          });
        }
      }
    };
